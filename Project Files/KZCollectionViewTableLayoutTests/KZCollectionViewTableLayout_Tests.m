//
//  KZCollectionViewTableLayoutTests.m
//  KZCollectionViewTableLayoutTests
//
...
#define KZValuedSizeMake(width, height) [NSValue valueWithCGSize:CGSizeMake(width, height)]
#define KZSizeIgnored CGSizeMake(CGFLOAT_MAX, CGFLOAT_MIN)

static NSString * const kKZHeaderColumnSizes = @"kKZHeaderColumnSizes";
static NSString * const kKZHeaderRowSizes = @"kKZHeaderRowSizes";

@interface KZCollectionViewTableLayout_Tests : XCTestCase
@property (nonatomic, strong) KEZCollectionViewTableLayout *layout;
@property (nonatomic, strong) UICollectionView *collectionView;
@property (nonatomic, strong) id delegate;
@end

@implementation KZCollectionViewTableLayout_Tests
#pragma mark - Initialization
#pragma mark Default Cell Size
- (void) testInit_setsDefaultCellSizeTo100x100 {
  [self setupWithoutDelegate];
  assertThat([NSValue valueWithCGSize:self.layout.cellSize], equalTo(KZValuedSizeMake(100, 100)));
}
...
- (void) testCollectionViewContentSize_withOneRow_adheresToSizing {
  ...
  assertThat([NSValue valueWithCGSize:contentSize], equalTo(KZValuedSizeMake(2000, 100)));
}
...
- (void) testCollectionViewContentSize_withManyRows_adheresToSizing {
  ...
  CGSize contentSize = self.layout.collectionViewContentSize;
  assertThat([NSValue valueWithCGSize:contentSize], equalTo(KZValuedSizeMake(5000, 300)));
}
...
- (void) testCollectionViewContentSize_withHeaders_withSingleRow_adheresToSizing {
  ...
  CGSize contentSize = self.layout.collectionViewContentSize;
  assertThat([NSValue valueWithCGSize:contentSize], equalTo(KZValuedSizeMake(2050, 250)));
}
...
- (void) testCollectionViewContentSize_withHeaders_withMultipleRows_adheresToSizing {
  ...
  CGSize contentSize = self.layout.collectionViewContentSize;
  assertThat([NSValue valueWithCGSize:contentSize], equalTo(KZValuedSizeMake(5050, 450)));
}
...
- (void) testCollectionViewContentSize_isNotLimitedByMinimumSize {
  ...
  CGSize contentSize = self.layout.collectionViewContentSize;
  assertThat([NSValue valueWithCGSize:contentSize], equalTo(KZValuedSizeMake(2000, 200)));
}
...
- (void) testCollectionViewContentSize_isNotLimitedByMaximumSize {
  ...
  CGSize contentSize = self.layout.collectionViewContentSize;
  assertThat([NSValue valueWithCGSize:contentSize], equalTo(KZValuedSizeMake(6000, 600)));
}
...
- (void) testCollectionViewContentSize_withDelegate_adheresToSizing {
  [self configureDelegatedLayoutWithCellSizes:@[
                                                @[KZValuedSizeMake(200.0f, 100.0f), KZValuedSizeMake(100.0f, 100.0f)],
                                                @[KZValuedSizeMake(300.0f, 100.0f), KZValuedSizeMake(500.0f, 200.0f), KZValuedSizeMake(100.0f, 150.0f)],
                                                @[KZValuedSizeMake(170.0f, 50.0f)]
                                                ]];
  CGSize contentSize = self.layout.collectionViewContentSize;
  assertThat([NSValue valueWithCGSize:contentSize], equalTo(KZValuedSizeMake(900.0f, 350.0f)));
}
...
- (void) testCollectionViewContentSize_withDelegate_isLimitedByMinimumSize {
  [self configureDelegatedLayoutWithCellSizes:@[
                                  ...
                                  maxCellSize:KZSizeIgnored];
  CGSize contentSize = self.layout.collectionViewContentSize;
  assertThat([NSValue valueWithCGSize:contentSize], equalTo(KZValuedSizeMake(1000.0f, 600.0f)));
}
...
- (void) testCollectionViewContentSize_withDelegate_isLimitedByMaximumSize {
  [self configureDelegatedLayoutWithCellSizes:@[
                                  ...
                                  minCellSize:KZSizeIgnored
                                  maxCellSize:CGSizeMake(200.0f, 100.0f)];
  CGSize contentSize = self.layout.collectionViewContentSize;
  assertThat([NSValue valueWithCGSize:contentSize], equalTo(KZValuedSizeMake(500.0f, 250.0f)));
}
...
- (void) testLayoutAttributesForItemAtIndexPath_withDelegate_adheresToSizing {
  [self configureDelegatedLayoutWithCellSizes:@[
                                                @[KZValuedSizeMake(50, 100),KZValuedSizeMake(150, 80)],
                                                @[KZValuedSizeMake(500, 100),KZValuedSizeMake(20, 200),KZValuedSizeMake(50, 50)]
                                                ]];
...
- (void) testLayoutAttributesForItemAtIndexPath_withDelegate_withHeaders_adheresToSizing {
  [self configureDelegatedLayoutWithCellSizes:@[
                                  ...
                                  headerSizes:@{
                                                kKZHeaderColumnSizes: @[KZValuedSizeMake(100, 200), KZValuedSizeMake(100, 250), KZValuedSizeMake(100, 100)],
                                                kKZHeaderRowSizes: @[KZValuedSizeMake(150, 100), KZValuedSizeMake(50, 300)]
                                                }];
...
- (void) configureDelegatedLayoutWithCellSizes:(NSArray *)sizes {
  [self configureDelegatedLayoutWithCellSizes:sizes minCellSize:KZSizeIgnored maxCellSize:KZSizeIgnored];
}

- (void) configureDelegatedLayoutWithCellSizes:(NSArray *)sizes headerSizes:(NSDictionary *)headerSizes {
  [self configureDelegatedLayoutWithCellSizes:sizes headerSizes:headerSizes minCellSize:KZSizeIgnored maxCellSize:KZSizeIgnored];
}
...
- (void) configureDelegatedLayoutWithCellSizes:(NSArray *)sizes headerSizes:(NSDictionary *)headerSizes minCellSize:(CGSize)minSize maxCellSize:(CGSize)maxSize {
  if ([headerSizes objectForKey:kKZHeaderColumnSizes] == nil && [headerSizes objectForKey:kKZHeaderRowSizes] == nil)
    [self setupWithCellDelegate];
  else
    [self setupEntirely];
  
  if (!CGSizeEqualToSize(minSize, KZSizeIgnored))
    self.layout.minimumCellSize = minSize;
  
  if (!CGSizeEqualToSize(maxSize, KZSizeIgnored))
    self.layout.maximumCellSize = maxSize;
  
  KZTableLayoutCellAndHeaderDelegate *delegate = self.delegate;
  
  if (headerSizes) {
    NSArray *columnSizes = [headerSizes objectForKey:kKZHeaderColumnSizes];
    ...
    NSArray *rowSizes = [headerSizes objectForKey:kKZHeaderRowSizes];
...
- (void) setupWithCellDelegate {
  self.layout = [[KEZCollectionViewTableLayout alloc] init];
  self.collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, 1000, 1000) collectionViewLayout:self.layout];
  self.delegate = [[KZTableLayoutCellDelegate alloc] init];
...
- (void) setupEntirely {
  self.layout = [[KEZCollectionViewTableLayout alloc] init];
  self.collectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, 1000, 1000) collectionViewLayout:self.layout];
  self.delegate = [[KZTableLayoutCellAndHeaderDelegate alloc] init];
  self.collectionView.delegate = self.delegate;
}
